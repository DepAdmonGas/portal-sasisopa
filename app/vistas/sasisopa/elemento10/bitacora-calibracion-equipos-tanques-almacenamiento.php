...
$UV = $class_control_actividad_proceso->detalleCalibracion($GET_ID,'Unidad de verificación');
$NA = $class_control_actividad_proceso->detalleCalibracion($GET_ID,'No. de acreditación');
$MUC = $class_control_actividad_proceso->detalleCalibracion($GET_ID,'Método usado para la calibración');
...
<html lang="es">
  ...
  function EditarOtros(e,id,input){
    ...
    var parametros = {
      "accion" : "editar-calibracion-equipos-tanque-almacenamiento",
      "contenido" : e.value,
      "id" : id,
      "input" : input
    };
 ...
 success:  function (response) {
  ...
  $('#Modal').modal('show');
  $('#ContenidoModal').load('../app/vistas/sasisopa/elemento10/modal-agregar-resultados-calibracion-tanques.php?ID=' + id);
  }
  ...
  data.append('accion', 'agregar-resultados-calibracion-tanque');
  data.append('id', id);

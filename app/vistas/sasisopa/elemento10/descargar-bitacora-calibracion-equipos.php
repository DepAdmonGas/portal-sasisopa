...
$contenid0 .= '<div style="page-break-before: always;"> </div>';
    ...
    $contenid0 .= '<table class="table table-sm table-bordered" style="font-size: .8em;">';
      ...
      $sql = "SELECT id,equipo FROM tb_calibracion_equipos WHERE id_estacion = '".$Session_IDEstacion."' AND estado = 1 ORDER BY fecha DESC ";
      $result = mysqli_query($con, $sql);
      $numero = mysqli_num_rows($result);
      while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
...
$dompdf->stream('Bitácora calibración de equipos.pdf');

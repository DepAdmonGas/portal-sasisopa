...
ON tb_cursos_calendario.id_personal = tb_usuarios.id WHERE tb_cursos_calendario.id_estacion  = '".$Session_IDEstacion."' AND tb_cursos_temas.categoria = 'SGM' AND $estadoCurso  ORDER BY tb_cursos_calendario.fecha_programada ASC ";
...
<div class="row mt-1">
	...
	<div class="col-6">
		...
		if ($numeroCurso > 0) {
		echo '<ul class="list-group rounded-0">';
		...
		$Titulo = '<div class="text-danger text-right"><b>Pendiente</b></div>';

		}else{
		if($rowCurso['resultado'] >= 90 && $rowCurso['resultado'] <= 100){
		$Titulo = '<div class="text-success text-right"><b>'.$rowCurso['resultado'].' (Excelente)</b></div>';
		}else if($rowCurso['resultado'] >= 80 && $rowCurso['resultado'] <= 89){
		$Titulo = '<div class="text-primary text-right"><b>'.$rowCurso['resultado'].' (Bueno)</b></div>';
		}else if($rowCurso['resultado'] >= 60 && $rowCurso['resultado'] <= 79){
		$Titulo = '<div class="text-warning text-right"><b>'.$rowCurso['resultado'].' (Regular)</b></div>';
		}else{
		$Titulo = '<div class="text-danger text-right"><b>'.$rowCurso['resultado'].' (Malo)</b></div>';
		...
		<div><b>'.$rowCurso['nombres'].' '.$rowCurso['apellido_p'].' '.$rowCurso['apellido_m'].'</b></div>

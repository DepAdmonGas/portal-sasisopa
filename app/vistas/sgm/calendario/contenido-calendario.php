...
INNER JOIN us_usuarios 
ON tb_cursos_calendario.id_personal = us_usuarios.id
WHERE tb_cursos_calendario.id_estacion = '".$idEstacion."' AND tb_cursos_calendario.fecha_programada = '".$CalenDate."' AND tb_cursos_temas.categoria = 'SGM' ";
$result = mysqli_query($con, $sql);
$numero = mysqli_num_rows($result);
while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
...
function generar_calendario($idEstacion,$month,$year,$lang,$con){
    ...
    for($list_day = 1; $list_day <= $days_in_month; $list_day++):
        ...
        $TotalActividad = $Cursos['Total'];
        $TotalFinalizadas = $Cursos['Finalizadas'];
        $TotalPendientes = $Cursos['Pendientes'];
...
<div class="border-0 p-3 bg-white"> 

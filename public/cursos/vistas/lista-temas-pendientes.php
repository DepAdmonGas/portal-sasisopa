...
ON tb_cursos_calendario.id_tema = tb_cursos_temas.id WHERE tb_cursos_calendario.id_personal = '".$Session_IDUsuarioBD."' AND tb_cursos_calendario.estado = 0 AND tb_cursos_calendario.fecha_programada <= '".$fecha_del_dia."' ORDER BY tb_cursos_calendario.fecha_programada ASC "; 
...
echo '<span class="badge bg-primary">Total pendientes: <b>'.$numero_modulos_cursos.'</b></span>';
...
<div class="text-end text-secondary"><small>'.FormatoFecha($row_modulos_cursos['fecha_programada']).'</small></div>
<div class="text-center mt-1"><b>'.$row_modulos_cursos['num_tema'].". ".$row_modulos_cursos['titulo'].'</b></div>
<div class="text-end mt-2"><button class="btn btn-sm btn-success" type="button" onclick="IniciarTema('.$row_modulos_cursos['id'].')">Inciar tema</button></div>
</div>
</div>';
}
echo '</div>';

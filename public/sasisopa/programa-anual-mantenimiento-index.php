...
$sql_sasisopa_ayuda = "SELECT * FROM pu_sasisopa_ayuda WHERE id_usuario = '".$Session_IDUsuarioBD."' and detalle = 'programa-anual-mantenimiento' and estado = 0 LIMIT 1";
...
<html lang="es">
  ...
  function ListaProgramaAnual(){
    $('#DivListaPrograma').load('public/sasisopa/vistas/lista-year-programa-anual-mantenimiento.php?idEstacion=<?=$Session_IDEstacion;?>');    
  }
  ...
  function ProgramaNew(id){  
    ...
    "fecha" : id
     ...
     $.ajax({
     data:  parametros,
     url:   'public/sasisopa/agregar/agregar-year-programa-anual.php',
     type:  'post',
     beforeSend: function() {
     },
     complete: function(){
 ...
 var parametros = {
  ...
  if (puntosSasisopa != 0) {
   ...
   $('#myModalProgramaAnual').modal('hide');  
    <?php if ($new_programa == 0) {echo "$(document).ready(function(){ ProgramaNew(".$year.") });";} ?>
   }
   });  
  ...
  <body>
    ...
    <a class="ml-2" onclick="ProgramaNew(<?=$year;?>)" style="cursor: pointer;" data-toggle="tooltip" data-placement="left" title="Agregar" >
